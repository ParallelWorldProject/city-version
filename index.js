...
class Game {
    constructor(total_ppl, total_patients, total_neigh, total_hospital, total_quarantine, total_office) {
        ...
        this.neighbors = [];
        this.hospitals = [];
        for (let i = 0; i < office_number; i++) { let o = new office();
            this.offices.push(o); }
        for (let i = 0; i < neighbor_number; i++) { let o = new neighbor();
            this.neighbors.push(o); }
        for (let i = 0; i < hospital_number; i++) { let o = new hospital();
            this.hospitals.push(o); }
        this.people = [];
        this.ids = new Set();
        //this.pplSet=new Set();
    ...
    updateGameStatus() {
        ...
        this.total_ppl = ttt[0];
        this.total_healthy = ttt[1];
        this.total_infected = ttt[2];
        this.total_cured = ttt[7];
...
function contagion(ids) //set of people's id who participate in this stage of contagion
{
    ...
    var x = 0,
        y = 0,
        t = ids.size; //分别是 传染源， 未传染人数，环节总人数
    for (var ttt of ids)
    ...
    for (var ttt of ids) {
        ...
        { game.people[ttt].health_status = 1;
            ret++; }
    }
    return ret;
}
...
function updateHealthStatus() {
    ...
    var one_to_two = 0.01,
        two_to_three = 0.01,
        markov_chain = [
            [0.97, 0.01 - delta / 2, 0.01 - delta / 2, 0.01 + delta],
    ...
    for (let tttt = 0; tttt < people_number; tttt++) {
        ...
        switch (cur.health_status) {
            ...
            case 4:
                ...
                break;
            case 2:
                if (Math.random() < two_to_three) cur.health_status++;
                break;
            case 1:
                if (Math.random() < one_to_two) cur.health_status++;
                break;
...
function gameLoop() //after the players' operations in the evening, the game loop begins in the next morning
{
    ...
    for (var cur of game.ids)
        if (game.people[cur].health_status == 4 || game.people[cur].health_status == 5) { sss.add(cur);
            game.hospitals[0].patient_count++; }
